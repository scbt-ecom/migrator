...
// RollbackMigrations выполняет откат всех или определённых миграций
func (m *Migrator) RollbackMigrations(versions ...string) error {
	if err := m.Lock(); err != nil {
		return fmt.Errorf("unable to acquire lock: %w", err)
	}
	defer m.Unlock()
